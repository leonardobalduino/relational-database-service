...
class DateCodec(TypeCodec):  # pragma: no cover
    python_type = date
    bson_type = datetime
...
class DecimalCodec(TypeCodec):  # pragma: no cover
    python_type = Decimal
...
def create_mongo_client_db() -> Tuple[MongoClient, Database]:
    client: MongoClient = AsyncIOMotorClient(settings.mongo.url_connection)
...
client_mongo, database = create_mongo_client_db()
